...
--[[
	...
	FIX:	Issue of sess.skillpoints being stored as decimal values instead of whole numbers
				this caused et.G_XP_set to only grant the first stat BATTLESENSE
	UPD:	Cleaned up code for efficiency (IMO)
	UPD:	Added JSONPath variable
	ADD:	Server XP reset timer using server admin variable XP_RESET_FLAG -- In Progress
	ADD:	Trigger switch variable to track or not track OmniBot XP
...
--]]
local JSON_PATH				= "etmain/lua" -- Path relative to fs_homepath, no preceding or trailing / required
...
--[[
			DO NOT MODIFY BELOW
--]]
local readPath				= string.gsub(et.trap_Cvar_Get("fs_homepath") .. "/" .. JSON_PATH .. "/","\\","/")
local writePath				= string.gsub(et.trap_Cvar_Get("fs_homepath") .. "/" .. et.trap_Cvar_Get("fs_game") .. "/","\\","/")

local JSON						= (loadfile(readPath .. JSON_FILE))()
...
-- This table of functions was added by Klassifyed to round decimal to whole numbers.
-- numbers returned from sess.skillpoints in function _setSkillPoints.
-- helper function because all the skill points would have up to 12 decimal places
local myMath = {
	...
	-- roundDown returns the largest integer smaller than or equal to num
	roundDown = function (num)
		return math.floor(num)
	end,

	-- roundUp returns the smallest integer larger than or equal to num
...
function _validateGUID (clientNum, guid)
	...
	if ( guid == nil ) or ( string.match(guid, "%W") ) or ( string.lower(guid) == "no_guid" ) or ( string.lower(guid) == "unknown" ) or ( string.len(guid) < 32 ) then -- Invalid characters detected.
		...
		message("cp", "^1WARNING: ^7Your XP won't be saved because you have an invalid cl_guid.", clientNum)
		return false
	end
...
-- This function was added by Klassifyed to help identify OMNIBOT GUIDs
...
function _printFinger (clientNum, targetNum)
	...
	if ( et.gentity_get(clientNum, "sess.referee") == 1 ) then
		...
		message("chat", "^ofinger: ^7Fingered info for " .. tostring(name), clientNum)
		message("print", "IP(" .. tostring(ip) .. ") GUID(" .. tostring(guid) .. ") QPORT(" .. tostring(port), clientNum)
		message("print", "ETVERSION(" .. tostring(etversion) .. ") PROTOCOL(" .. tostring(protocol), clientNum)
	else
		message("chat", "^ofinger: ^7You do not have access to this command", clientNum)
	end
end
...
function _advPlayers (clientNum)
	message("print", "^3 ID ^1: ^3Player                     Rate  Snaps", clientNum)
	message("print", "^1--------------------------------------------", clientNum)
	local team = {
		"^1X", -- Axis
		"^4L", -- Allies
		"^3S"  -- Spectator
	...
	for i=0, tonumber(et.trap_Cvar_Get("sv_maxclients"))-1 do
		...
		if ( et.gentity_get(i, "pers.connected") == 2 ) then
			message("print", string.format("%s^7%2s ^1:^7 %s%s %5s  %5s %s", team[teamNumber], i, name, space, rate, snaps, ref), clientNum)
...
function et_ClientCommand (clientNum, command)
	...
	if ( Arg0 == "say" )  then
		...
		elseif ( Arg1 == "!loadxp" ) then
			_loadXp(clientNum)
			message("cp", "^oLoadXp: ^7Your xp has been loaded", clientNum)
			return 1
		elseif ( Arg1 == "!savexp" ) then
			_saveXp(clientNum)
			message("cp", "^oSaveXp: ^7Your xp has been saved", clientNum)
			return 1
		elseif ( Arg1 == "!resetxp" ) then
			_resetXp(clientNum)
			message("cp", "^oResetXp: ^7Your xp has been reset", clientNum)
			return 1
...
function et_ClientBegin (clientNum)
	...
	message("cpm", "^3Welcome ^7" .. name .. "^3! You are playing on an XP save server", clientNum)
end
